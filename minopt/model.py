...
from minopt.utils import download_sharded_weights
...
def get_mask(
    ...
    *,
    ...
    """Returns the self-attention mask for forward decoding.

    Args:
        tsz: The time length of the mask
        attn_type: The type of attention to use
        device: The device for the newly-created tensor
        dtype: The dtype for the newly-created tensor

    Returns:
        The attention mask, with shape (tsz, tsz)

    Raises:
        NotImplementedError: Invalid attention type
    """

    # The two mask functions below generate the same tensor.
    # mask = ~torch.tril(torch.ones(tsz, tsz, device=device))
    if attn_type == "linear":
        return torch.tril(torch.ones(tsz, tsz, device=device, dtype=dtype), diagonal=0)
    if attn_type in ("softmax", "sigmoid"):
        return torch.triu(
            torch.full((tsz, tsz), float("-inf"), device=device, dtype=dtype),
            diagonal=1,
        )
...
def linear_attention(
    ...
    if mask is not None:
        kq = kq * mask
    attn = kq / kq.sum(dim=-1, keepdim=True).clamp_min(1.0)
    out = attn @ v  # (..., Tq, C)
    return out, attn
...
def attention(
    ...
    """Performs linear attention computation.

    Args:
        q: Query tensor with shape (B, C, Tq)
        k: Key tensor with shape (B, C, Tk)
        v: Value tensor with shape (B, C, Tk)
        attn_type: The attention type to use, (linear, sigmoid or softmax)
    ...
    Raises:
        NotImplementedError: If the attention type is invalid
    """

    if attn_type == "linear":
        return linear_attention(q, k, v, mask)
    if attn_type == "sigmoid":
        return sigmoid_attention(q, k, v, mask)
    if attn_type == "softmax":
        return softmax_attention(q, k, v, mask)
    raise NotImplementedError(f"Invalid attention type: {attn_type}")
...
class FullyConnected(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.conv(x)
        x = self.norm(x)
        x = self.act(x)  # pylint: disable=not-callable
        return x
    ...
    def __init__(
        ...
        *,
        ...
        if (projection_dims := (channels * mult)) % parallel_world_size != 0:
            raise ValueError(
                f"Cannot divide {projection_dims=} among {parallel_world_size=}"
            )
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        out, _ = attention(q, k, v, self.attn_type, mask)
        # (B, H, T, C / H) -> (B, C, T)
...
class OPT(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        mask = get_mask(max_position_embeddings, attn_type="softmax")
